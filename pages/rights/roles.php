...
<div class="bootstrap-wrapper">
    ...
    <table class="table table-hover">
        <thead>
            <tr>
                ...
                <th><?php _e("Anzeige Namen", ClubCore::$TEXT_DOMAIN) ?></th>
                <th><?php _e("Erbt von", ClubCore::$TEXT_DOMAIN) ?></th>
                <th>&nbsp;</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach (Role::findAll() as $key => $role) { ?>
                <tr>
                    <td id="name_<?php echo $role->rid ?>"><?php echo $role->name ?></td>
                    <td id="displayName_<?php echo $role->rid ?>"><?php echo $role->displayName ?></td>
                    <td id="parrent_<?php echo $role->rid ?>"><?php echo $roles[$role->parrent] ?></td>
                    <td>
    ...
    <br />
    <button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#mdlNew"><?php _e("Neu", ClubCore::$TEXT_DOMAIN) ?></button>
    <div class="modal fade" id="mdlNew" tabindex="-1" role="dialog" aria-labelledby="mdlNewLabel" aria-hidden="true">
                ...
                <div class="modal-body">
                    <form role="form">
                        <div class="form-group">
                            <label for="txtNameEdit"><?php _e("Namen", ClubCore::$TEXT_DOMAIN) ?></label>
                            <input type="text" class="form-control" id="txtNameEdit" placeholder="<?php _e("Namen", ClubCore::$TEXT_DOMAIN) ?>" />
                        </div>
                        <div class="form-group">
                            <label for="txtDisplaNameEdit"><?php _e("Anzeige Namen", ClubCore::$TEXT_DOMAIN) ?></label>
                            <input type="text" class="form-control" id="txtDisplaNameEdit" placeholder="<?php _e("Anzeige Namen", ClubCore::$TEXT_DOMAIN) ?>" />
                        </div>
                        <div class="form-group">
                            <label for="selParrentEdit"><?php _e("Erbt von", ClubCore::$TEXT_DOMAIN) ?></label>
                            <select id="selParrentEdit" class="form-control">
                                <?php foreach ($roles as $key => $r) { ?>
                                    <option value="<?php echo $key ?>"><?php echo $r ?></option>
                                <?php } ?> 
                            </select>
                        </div>
                    </form>
